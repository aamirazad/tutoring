...
function createFileNode(
	...
	const isProtectedFile = node.slugSegment.endsWith("-protected");

	if (isProtectedFile) {
		a.classList.add("explorer-protected-link");
		a.appendChild(createProtectedLockIcon());
	}

	a.href = resolveRelative(currentSlug, node.slug);
	a.dataset.for = node.slug;
	a.append(node.displayName);
...
function createProtectedLockIcon(): SVGSVGElement {
	...
	svg.setAttribute(
		"class",
		"lucide lucide-lock-keyhole-icon lucide-lock-keyhole explorer-protected-icon",
	);
	...
	path.setAttribute("d", "M7 10V7a5 5 0 0 1 10 0v3");

	svg.append(circle, rect, path);
	return svg;
}
